...
class CarsManager:
    def __init__(self, game_mode: GameMode, tile_map: TileMap, entity_manager: EntityManager,
        ...
        self._npc_rects: list[pygame.Rect] = []
    ...
    def update_cars(self, delta_time: float):
        ...
        for i, car in enumerate(self._cars):
            ...
            tile_of_car: Tile = self._tile_map.get_tile_at_pos_vec(car_transform.get_position())

            self.handle_ai_knowledge(car, tile_of_car)
    ...
    def handle_ai_knowledge(self, car: Car, tile_of_car: Tile):
        ...
        # self._npc_rects = [self._entity_manager.get_collider(npc.entity_ID).rect for npc in npcs]

        fov.update(car_transform, self._tile_map, self._npc_transforms, self._npc_rects)
    ...
    def render_debug(self):
        for car in self._cars:
            ...
            self._debug_renderer.draw_text_absolute(f"Checkpoint number: {car.car_knowledge.checkpoint_number}",
